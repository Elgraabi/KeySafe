...
type AuthContextType = {
    ...
    forgotPassword: (username: string, newPassword: string) => Promise<{ success: boolean; message: string }>;
    logout: () => Promise<void>;
...
const AuthContext = createContext<AuthContextType>({
    ...
    login: async () => { },
    register: async () => { },
    forgotPassword: async () => { return { success: false, message: "" }; },
    logout: async () => { },
    resetData: async () => { },
    editProfile: async () => { },
});
...
const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    const loadStoredData = async () => {
        try {
            ...
            const keepConnectedStored = await AsyncStorage.getItem(STORAGE_KEEP_CONNECTED_KEY);
        ...
        try {
            ...
            if (userStored) {
                ...
                if (username === user.username) {
                    ...
                    setIsAuthenticated(true);
                    await AsyncStorage.setItem(STORAGE_KEEP_CONNECTED_KEY, JSON.stringify(keepConnected));
                } else {
    ...
    const register = async (name: string, username: string, password: string) => {
        try {
            const newUser: UserType = { name, username };
    ...
    const forgotPassword = async (username: string, newPassword: string): Promise<{ success: boolean; message: string }> => {
        try {
            const usersStored = await AsyncStorage.getItem(STORAGE_USER_KEY); // Usar o mesmo local do usuário armazenado
            const users = usersStored ? [JSON.parse(usersStored)] : []; // Aqui você pode simular uma lista com um único usuário para testes
            ...
            await AsyncStorage.setItem(STORAGE_USER_KEY, JSON.stringify(users[userIndex]));
    ...
    const logout = async () => {
        try {
            ...
            Alert.alert("Logout", "Não foi possível sair. Tente novamente mais tarde.");
        }
    };
    ...
    const editProfile = async (name: string) => {
        try {
            ...
            Alert.alert("Perfil", "Não foi possível editar o perfil. Tente novamente mais tarde.");
        }
    };
